...
class Solution {
    func isKeyPresentInEveryKSegment(_ arr: [Int], x: Int, k: Int) -> Bool {
        var startIndex = 0
        var endIndex = 0
        guard arr.count >= k - 1 else {
            return false
        }
        while startIndex <= arr.count - k {
            endIndex = startIndex + k - 1
            var keyFound = false
            for windowIndex in startIndex...endIndex {
                if arr[windowIndex] == x {
                    keyFound = true
                    break
                }
            }
        ...
        if startIndex <= arr.count - 1 {
            var keyFound = false
            for windowIndex in startIndex...(arr.count - 1) {
                if arr[windowIndex] == x {
                    keyFound = true
                    break
                }
...
let sol = Solution()
sol.isKeyPresentInEveryKSegment([ 3, 5, 2, 4, 9, 3, 1, 7, 3, 11, 12, 3], x: 3, k: 3)
sol.isKeyPresentInEveryKSegment([ 21, 23, 56, 65, 34, 54, 76, 32, 23, 45, 21, 23, 25], x: 23, k: 5)
sol.isKeyPresentInEveryKSegment([ 5, 8, 7, 12, 14, 3, 9], x: 8, k: 2)
sol.isKeyPresentInEveryKSegment([], x: 8, k: 2)
sol.isKeyPresentInEveryKSegment([1, 2, 3], x: 3, k: 3)
